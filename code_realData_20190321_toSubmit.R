...
rm(list=ls())
origin<-"C:\\Users\\user\\Dropbox\\임요한\\peeling_JSLEE\\"
direc_in<-origin
direc_out<-paste(origin, "test20220805\\", sep="")

direc_data<-"C:\\Users\\user\\Dropbox\\임요한\\peeling_JSLEE\\DATA\\"
...
setwd(direc_in)
source("ftn_by_lee_new.R")
source("ftn_by_JY_20170508_20190531_tosubmit.R")
...
if(case==1){
	...
	source("C:\\Users\\user\\Dropbox\\임요한\\peeling_JSLEE\\DATA\\omija2\\omija.R")
	...
	hiv<-scan(paste(direc_data, "hivdata.txt", sep=""))
	data<-hiv
	outfile<-"hiv"
}else if(case==3){
	###########################################################################
	####  Data 3: Epidemic data                                            ####
	####  Downloaded from https://websenti.u707.jussieu.fr/sentiweb/?page=table
	###########################################################################

	ili<-read.csv(paste(direc_data, "ILI-France.csv",sep=""), skip=1, as.is=T)
...
my_y_at<-NULL
if(case==1){
	outfile<-"omija_hist_ver_4.pdf"
	my_y_lim<-c(0, 0.5)
	my_x_at<--6.8
	my_y_at<-0.49
	}
if(case==2){
	outfile<-"hiv_hist_ver_4.pdf"
	my_y_lim<-c()
